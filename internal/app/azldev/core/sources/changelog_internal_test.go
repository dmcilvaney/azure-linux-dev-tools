...
func TestTryMaterializeStaticChangelog_ManualSkips(t *testing.T) {
	...
	comp := mockComponent(ctrl, "kernel", &projectconfig.ComponentConfig{
		Autospec: projectconfig.AutospecConfig{
			ChangelogCalculation: projectconfig.ChangelogCalculationManual,
		},
	})
	...
	for _, clCalc := range nonManualModes {
		...
		t.Run(name, func(t *testing.T) {
			...
			comp := mockComponent(ctrl, "aalib", &projectconfig.ComponentConfig{
				Autospec: projectconfig.AutospecConfig{
					ReleaseCalculation:   projectconfig.ReleaseCalculationManual,
					ChangelogCalculation: clCalc,
				},
			})

			err := preparer.tryMaterializeStaticChangelog(comp, testSourcesDir, "")
			require.Error(t, err)
			assert.Contains(t, err.Error(), `autospec.release-calculation = "manual"`)
...
func TestTryMaterializeStaticChangelog_AutochangelogSkips(t *testing.T) {
	...
	comp := mockComponent(ctrl, "test-pkg", &projectconfig.ComponentConfig{
		Autospec: projectconfig.AutospecConfig{
			ChangelogCalculation: projectconfig.ChangelogCalculationAutochangelog,
		},
	})
...
func TestTryMaterializeStaticChangelog_AutoSkipsForAutochangelog(t *testing.T) {
	...
	comp := mockComponent(ctrl, "test-pkg", &projectconfig.ComponentConfig{
		Autospec: projectconfig.AutospecConfig{
			ChangelogCalculation: projectconfig.ChangelogCalculationAuto,
		},
	})
...
func TestTryMaterializeStaticChangelog_ExplicitStaticMaterializes(t *testing.T) {
	...
	comp := mockComponent(ctrl, "test-pkg", &projectconfig.ComponentConfig{
		Autospec: projectconfig.AutospecConfig{
			ChangelogCalculation: projectconfig.ChangelogCalculationStatic,
		},
	})
...
func TestTryMaterializeStaticChangelog_ExplicitStaticErrorsOnAutochangelog(t *testing.T) {
	...
	err := preparer.tryMaterializeStaticChangelog(comp, filepath.Join(testSourcesDir, "test-pkg"), "")
	require.Error(t, err)
	assert.Contains(t, err.Error(), `autospec.changelog-calculation = "autochangelog"`)
}
...
func TestChangelogBodyUsesAutochangelog(t *testing.T) {
	for _, testCase := range []struct {
		...
		// Hybrid pattern common in Fedora upstream: one or more static entries
		// followed by %autochangelog. azldev must detect this and skip its
		// migration steps so rpmautospec can expand the macro as the maintainer
		// authored it. The detection must scan the entire body, not just the
		// first non-blank line.
		{"hybrid static then autochangelog", []string{
			"* Mon Jan 01 2024 Someone - 1-1",
			"- entry.",
			"",
			"%autochangelog",
		}, true},
		{"hybrid braced", []string{
			"* Mon Jan 01 2024 Someone - 1-1",
			"- entry.",
			"%{autochangelog}",
		}, true},
		{"hybrid conditional", []string{
			"* Mon Jan 01 2024 Someone - 1-1",
			"- entry.",
			"%{?autochangelog}",
		}, true},
