...
// Contract describes the rpmautospec invariants that every commit in a
// synthetic dist-git must satisfy, including the seed commit. The four
// invariants are:
//
//  1. Spec '%changelog' body = '%autochangelog' (when ReplaceChangelog is set)
//  2. Spec 'Release' tag = '%autorelease' (when ReplaceRelease is set)
...
//
// The seed commit (import-commit, idx==0) is also Materialized: keeping its
// original tree would leave a static→'%autochangelog' boundary at synth-1,
// which rpmautospec sees as 'changelog changed: True' and treats as a manual
// edit — dropping the first synth entry. By Materializing the seed, the
// boundary moves below our walk (to upstream's natural parent), where it
// reflects real history. rpmautospec walks back TO the seed and stops.
//
...
type Contract struct {
	// ReplaceRelease controls whether the spec's 'Release' tag is rewritten to
	...
	// ReplaceChangelog controls whether the spec's '%changelog' body is rewritten
...
func (c Contract) Materialize(
	...
	newSpecBlobHash, specChanged, err := replaceMacrosInSpecBlob(repo, specEntry, c.ReplaceRelease, c.ReplaceChangelog)
	if err != nil {
...
// replaceMacrosInSpecBlob rewrites the spec's '%changelog' body to '%autochangelog' and/or
// 'Release' to '%autorelease' depending on the replaceRelease and replaceChangelog
...
func replaceMacrosInSpecBlob(
	repo *gogit.Repository, specEntry *object.TreeEntry, replaceRelease, replaceChangelog bool,
	...
	if !replaceRelease && !replaceChangelog {
	...
	if replaceRelease {
		if err := ReplaceReleaseWithAutorelease(specFile); err != nil {
...
// safeShortHash returns the first length characters of hash, or the full hash
// if it is shorter. Use when hashes may legitimately be shorter than
// [shortHashLen] (e.g., hand-edited lock files) and a slice would panic.
