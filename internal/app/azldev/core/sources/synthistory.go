...
type FingerprintChange struct {
	...
	// (opt-in via ReleaseConfig.ReplayHistoricalOverlays). Nil when replay is
...
// (import-commit) is replayed as a root commit via [replaySeedWithContract];
// post-seed upstream commits are replayed with changelog attribution via
// [replayUpstreamWithContract]. All trees are materialized via [Contract]
// to satisfy the rpmautospec invariants.
...
func replayInterleavedHistory(
	...
	// Replay the seed (import-commit), preserving all of its upstream
	// parents. Attribution is skipped on the seed: its %changelog entries
	// are either captured in the sidecar (when wroteSidecar=true) or
	// generated by rpmautospec from the preserved upstream history.
	// Attributing here would duplicate them.
...
// contract-satisfying tree. Attribution is always skipped.
//
...
//
// Attribution: if the upstream commit added one or more entries to the
// spec's %changelog vs its upstream parent, the replay fans out into one
// synth commit per new entry, each authored by the entry's header (date +
// name + email) and bodied by the entry's lines. This preserves real
// upstream authorship in cases where the git commit was made by a bot or
// release-engineering account but the %changelog header credits the real
// author.
//
// Fallback (best-effort): when attribution is unavailable — root commit,
// missing spec, no %changelog section, parse failure, or no new entries
// — the upstream commit's git author/committer/message are used verbatim,
// preserving the historical behavior exactly (including distinct committer).
func replayUpstreamWithContract(
	...
	// Look up the upstream parent's tree for changelog attribution. Note this
	// is the *upstream* parent, not the synth-chain parent (which may have a
	// contract-rewritten tree). Diffing against the synth-chain parent would
	// produce spurious "all entries new" results.
	upstreamParentTree := lookupUpstreamParentTree(repo, commit)

	metas := AttributeFromTrees(repo, commit.TreeHash, upstreamParentTree)
	if len(metas) == 0 {
		// Fallback: replay verbatim with upstream author/committer/message.
		hash, err := createCommitObject(repo, newTree,
			commit.Author, commit.Committer, commit.Message,
			parentHash)
		if err != nil {
	...
	// Fan out: one synth commit per new %changelog entry, all sharing the
	// upstream commit's materialized tree. Commits 2..N are tree-identical
	// to commit 1 (empty diffs) — rpmautospec walks them as distinct
	// %autochangelog entries.
	current := parentHash

	for _, m := range metas {
		sig := signatureFromMetadata(m)

		hash, err := createCommitObject(repo, newTree, sig, sig, m.Message, current)
		if err != nil {
			return plumbing.ZeroHash, fmt.Errorf("failed to replay upstream commit (attributed):\n%w", err)
		}

		current = hash
	}

	slog.Debug("Attributed upstream commit from %changelog entries",
		"upstreamCommit", commit.Hash,
		"entryCount", len(metas))

	return current, nil
}

// lookupUpstreamParentTree returns the tree hash of commit's first parent,
// or [plumbing.ZeroHash] if commit is a root commit or the parent cannot be
// loaded. Used by changelog-attribution to diff %changelog entries between
// an upstream commit and its true upstream predecessor.
func lookupUpstreamParentTree(repo *gogit.Repository, commit *object.Commit) plumbing.Hash {
	...
	if err != nil {
		slog.Debug("changelog attribution: failed to load upstream parent commit",
			"commit", commit.Hash, "parent", commit.ParentHashes[0], "err", err)

		return plumbing.ZeroHash
	}

	return parentCommit.TreeHash
}

// signatureFromMetadata builds a git Signature from a CommitMetadata,
// using its day-resolution timestamp in UTC.
func signatureFromMetadata(m CommitMetadata) object.Signature {
	return object.Signature{
		Name:  m.Author,
		Email: m.AuthorEmail,
		When:  unixToTime(m.Timestamp),
	}
}
...
func buildSyntheticCommits(
	...
	if config.Release.ReplayHistoricalOverlays {
