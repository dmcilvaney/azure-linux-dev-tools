...
`
	...
	contract := sources.Contract{ReplaceRelease: true, ReplaceChangelog: true}
...
`
	...
	contract := sources.Contract{ReplaceRelease: true, ReplaceChangelog: true, SidecarBlob: sidecarHash}
...
func TestContract_CommitMessage_WithSkipChangelog(t *testing.T) {
	contract := sources.Contract{ReplaceRelease: true, ReplaceChangelog: true, SkipChangelog: true}
...
func TestContract_CommitMessage_WithoutSkipChangelog(t *testing.T) {
	contract := sources.Contract{ReplaceRelease: true, ReplaceChangelog: true, SkipChangelog: false}
...
func TestContract_Materialize_NoFlips_PreservesSpec(t *testing.T) {
	// Regression test: when both ReplaceRelease and ReplaceChangelog are false (manual
...
`
	...
	contract := sources.Contract{ReplaceRelease: false, ReplaceChangelog: false}
...
func TestContract_Materialize_OnlyReplaceRelease(t *testing.T) {
...
`
	...
	contract := sources.Contract{ReplaceRelease: true, ReplaceChangelog: false}
...
func TestContract_Materialize_OnlyReplaceChangelog(t *testing.T) {
...
`
	...
	contract := sources.Contract{ReplaceRelease: false, ReplaceChangelog: true}
...
// TestContract_Materialize_PreservesExistingAutoreleaseArgs verifies that
// Contract.Materialize does not strip rpmautospec args from a spec that
// already uses %autorelease. This is the production path: during synth-history
// replay every tree spec passes through Contract.Materialize, and historical
// commits may have authored their Release line with -b/-p/-e/-s flags.
// Stripping those flags silently corrupts NVRs across the rewritten history.
func TestContract_Materialize_PreservesExistingAutoreleaseArgs(t *testing.T) {
	cases := []string{
		"%autorelease -b 10",
		"%autorelease -p",
	...
	for _, releaseVal := range cases {
		t.Run(releaseVal, func(t *testing.T) {
			...
			// ReplaceRelease=true is what sourceprep passes for auto/static/autorelease
			// modes. The existing %autorelease args MUST be preserved.
			contract := sources.Contract{ReplaceRelease: true, ReplaceChangelog: true}
			...
			specOut := readSpecFromTree(t, repo, newTree)
			assert.Contains(t, specOut, "Release: "+releaseVal+"\n",
				"Contract.Materialize must preserve existing %%autorelease args")
		})
	}
}
