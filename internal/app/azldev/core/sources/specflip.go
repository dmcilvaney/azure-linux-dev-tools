...
// number from the synthetic git history. After flipping, the staged spec is
// handed to rpmautospec, which emits the resolved value inline; azldev does
// not read the resolved value back. If the spec has no 'Release:' tag,
...
// 'changelog' file contents below them, producing a single materialized
// '%changelog' block in the rendered spec that covers both synthetic and
...
// 'changelog' file alongside the spec in the synthetic dist-git, where
// rpmautospec process-distgit will append them below the entries it
// materializes from the synthetic git history.
//
// Returns [spec.ErrSectionNotFound] if the spec has no '%changelog' section.
