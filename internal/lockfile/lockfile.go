...
type ComponentLock struct {
	...
	// injects after that anchor during synthetic-history replay.
	//
	// Excluded from the input fingerprint to avoid double-counting with
	// the fingerprint walker. ManualBump (which IS fingerprinted) handles
	// ongoing --bump operations independently. If a rebuild is also needed
	// when writing Bumps, increment ManualBump in the same commit.
	//
