...
//  2. Render all specs with the NEW azldev binary (autorelease rewrite) into the
...
//
// After running this tool you MUST re-render with the new azldev binary before
// committing: the rendered specs in your working tree were produced in step 2
// without the bumps, so their Release numbers will be too low. Re-rendering
// applies the bumps that this tool just wrote into the locks. Verify with
// `git diff` that Release numbers in the rendered specs now match (or exceed)
// the values at HEAD before committing.
//
...
func processLock(
	...
	// Bump so the new Release strictly exceeds the old (old < new), preserving
	// monotonic NVR across the migration. Equal NVRs would let yum/dnf consider
	// the new build a no-op upgrade.
	gap := oldRelease - newRelease + 1

	if gap <= 0 {
		log.Printf("OK    %-30s  old=%d  new=%d  (no bump needed, new > old)",
			name, oldRelease, newRelease)

		return "no-bump"
	}

	log.Printf("BUMP  %-30s  old=%d  new=%d  gap=%d  resulting-release=%d  anchor=%s",
		name, oldRelease, newRelease, gap, newRelease+gap,
		safeShortHash(lock.UpstreamCommit, shortHashLen))

	if !dryRun {
		// NOTE: Deliberately NOT incrementing lock.ManualBump here. migrate-release
		// is a one-shot tool and Bumps is intentionally excluded from
		// InputFingerprint (lockfile.go's Bumps doc). No consumer in this org keys
		// on InputFingerprint for cache invalidation in a way that would be hurt
		// by a Bumps write without a fingerprint bump. If that ever changes, the
		// tool should be updated to also call lock.ManualBump++.
		...
		comment := fmt.Sprintf(
			"# Migration: static Release %d -> %%autorelease (base %d) at upstream commit %s. Bump = %d (new release = %d, > old).",
			oldRelease, newRelease, safeShortHash(lock.UpstreamCommit, 12), gap, newRelease+gap)
