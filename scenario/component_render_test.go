...
// TestRenderStaticChangelogFlippedToAutochangelog verifies that a local spec
// with a static %changelog section has its body flipped to %autochangelog
// during rendering, and rpmautospec materializes the changelog entries.
//
// Note: the Release tag flip to %autorelease only happens through the
...
func TestRenderStaticChangelogFlippedToAutochangelog(t *testing.T) {
	t.Parallel()
	...
	spec := projecttest.NewSpec(
		projecttest.WithName("flip-test"),
		projecttest.WithVersion("2.0.0"),
		projecttest.WithRelease("1%{?dist}"),
		projecttest.WithBuildArch(projecttest.NoArch),
	)
	...
	project := projecttest.NewDynamicTestProject(
		projecttest.AddSpec(spec),
		projecttest.AddComponent(localComponentConfig("flip-test")),
		projecttest.UseTestDefaultConfigs(),
		projecttest.WithGitRepo(),
	)
	...
	results := projecttest.NewProjectTest(
		project,
		[]string{"component", "render", "flip-test", "-o", "project/SPECS"},
		projecttest.WithTestDefaultConfigs(),
	).RunInContainer(t)
	...
	renderedSpecPath := results.GetProjectOutputPath("SPECS", "f", "flip-test", "flip-test.spec")
	require.FileExists(t, renderedSpecPath)
