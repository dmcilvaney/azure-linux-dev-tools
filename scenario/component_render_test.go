...
// TestRenderStaticChangelogReplacedWithAutochangelog verifies that a local spec
// with a static %changelog section has its body replaced with %autochangelog
// during rendering, and rpmautospec materializes the changelog entries.
//
// Note: the Release tag replacement with %autorelease only happens through the
// synth-history pipeline (upstream components with locks). Local components
// keep their static Release tag since rpmautospec needs synth-history
// commits to count for %autorelease.
func TestRenderStaticChangelogReplacedWithAutochangelog(t *testing.T) {
	t.Parallel()
	...
	spec := projecttest.NewSpec(
		projecttest.WithName("replace-test"),
		projecttest.WithVersion("2.0.0"),
		projecttest.WithRelease("1%{?dist}"),
		projecttest.WithBuildArch(projecttest.NoArch),
	)
	...
	project := projecttest.NewDynamicTestProject(
		projecttest.AddSpec(spec),
		projecttest.AddComponent(localComponentConfig("replace-test")),
		projecttest.UseTestDefaultConfigs(),
		projecttest.WithGitRepo(),
	)
	...
	results := projecttest.NewProjectTest(
		project,
		[]string{"component", "render", "replace-test", "-o", "project/SPECS"},
		projecttest.WithTestDefaultConfigs(),
	).RunInContainer(t)
	...
	require.Len(t, output, 1)
	assert.Equal(t, "ok", output[0]["status"],
		"Static changelog spec should render ok")

	renderedSpecPath := results.GetProjectOutputPath("SPECS", "r", "replace-test", "replace-test.spec")
	require.FileExists(t, renderedSpecPath)
	...
	// rpmautospec should have processed the spec and materialized the
	// changelog (even for local specs, the static %changelog gets
	// handled by the autochangelog pipeline).
	assert.Contains(t, specContent, "## START: Generated by rpmautospec",
		"rpmautospec should have materialized the changelog")
	assert.Contains(t, specContent, "%changelog",
		"rendered spec should still have a %%changelog section")
}
