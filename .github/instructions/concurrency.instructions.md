...
- Use `sync.Mutex` to protect shared mutable state (e.g., lock file updates)
- Write per-index results directly (`results[idx]`) — each goroutine owns its index, no lock needed
- Lock only for shared state mutations (e.g., `lock.SetUpstreamCommit`)
